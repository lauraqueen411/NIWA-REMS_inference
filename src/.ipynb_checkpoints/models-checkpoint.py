...
def res_linear_activation_v2_noise(input_size, resize_output, num_filters, kernel_size, num_channels, num_classes, resize=True,
    ...
    x = res_block_initial(x, [num_filters[3]], 3, [1, 1], f"noise_blockderere", sym_padding=True)
    ...
    x = tf.keras.layers.Concatenate(-1)([noise_updated, x])
    x = up_block(x, temp2, kernel_size=3, filters = num_filters[2], i =2, sym_padding=False)
    ...
    output = res_block_initial(output, [num_filters[1]], 3, [1, 1], "output_convbbb123456", sym_padding=True)
    output = res_block_initial(output, [num_filters[0]], 3, [1, 1], "output_convbbb1234", sym_padding=True)
